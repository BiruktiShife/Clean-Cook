  ...
  return (
    <motion.header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled
          ? "bg-transparent backdrop-blur-md border-b border-green-200/50 shadow-lg shadow-green-500/10"
          : "bg-transparent"
      }`}
      initial={{ y: -100 }}
      animate={{ y: 0 }}
    ...
    >
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 lg:h-20">
            ...
            {navItems.map((item, index) => (
              ...
              >
                ...
                {item.hasDropdown && activeDropdown === item.name && (
                  <AnimatePresence>
                    <motion.div
                      className="absolute top-full left-0 mt-2 w-56 bg-white/95 backdrop-blur-md rounded-lg shadow-xl border border-green-200/50 py-2 z-50"
          ...
          >
            <Button
              ...
              className="bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white shadow-lg shadow-green-500/25 hover:shadow-green-500/40 transition-all duration-300"
            >
              Get Started
            </Button>
          </motion.div>
        ...
        {isMobileMenuOpen && (
          ...
          >
            <div className="px-2 pt-2 pb-3 space-y-1 bg-black/20 backdrop-blur-md border-t border-green-200/50 shadow-lg">
              {navItems.map((item) => (
                <div key={item.name}>
                  <a
                    href={item.href}
                    ...
                    className="flex items-center justify-between px-4 py-3 text-base font-medium text-white hover:text-green-400 hover:bg-green-500/20 rounded-lg transition-colors duration-200"
                  >
                    {item.name}
                    ...
                    <div className="ml-4 mt-1 space-y-1">
                      {item.dropdownItems.map((dropdownItem) => (
                        <a
                          ...
                          className="block px-4 py-2 text-sm text-gray-300 hover:text-green-400 hover:bg-green-500/10 rounded-lg transition-colors duration-200"
                        >
                          {dropdownItem.name}
                        </a>
                      ))}
